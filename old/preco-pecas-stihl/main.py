...
for index, linha_df1 in df1.iterrows():
    ...
    if not linhas_encontradas.empty:
        ...
        try:
            ...
            valor_final_frete = valor_da_coluna_c_df2
        ...
        df1.loc[index, 'Preço Real Stihl'] = valor_da_coluna_c_df2
        
        indices_modificados.append(index)

# --- GERAÇÃO ARQUIVO.XLSX ---
def destacar_celulas(linha):
    colunas_alteradas = colunas_para_alterar_valor_c + [coluna_para_alterar_valor_i] + ['Novo Pr.Compra'] + ['Novo Frete Entrada']
...
try:
    ...
    colunas_para_formatar = colunas_para_alterar_valor_c + [coluna_para_alterar_valor_i] + ['Novo Pr.Compra'] + ['Novo Frete Entrada']
    ...
    # MUDANÇA: Adiciona a nova coluna na lista para ser apagada do arquivo Autcom
    ...
    mapeamento_posicoes = {
        ...
        'Novo IPI Entrada': 43, 
        'Novo Frete Entrada': 44,
        'Novo Pr. Venda 1': 55, 'Novo Pr. Venda 2': 58,
